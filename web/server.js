...
async function render(req, res) {
  ...
  try {
    const jobs = [];
    const needTask = ["task", "edit", "move", "reminder"].includes(r.page) && r.id != null;
    ...
    if (needTask) jobs.push(api(`/task_reminders_view?task_id=eq.${r.id}&order=next_fire_at.asc`)
...
async function act(req, res) {
  ...
  try {
    ...
    if (a === "reminder") await api("/task_reminders", {
      method: "POST", headers: H,
      body: JSON.stringify({ task_id: +b.task_id, before: String(b.before || "").trim() }),
    });
